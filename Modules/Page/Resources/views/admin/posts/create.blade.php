...
<h1>
    {{ trans('page::posts.create post') }}
</h1>
<ol class="breadcrumb">
    <li><a href="{{ URL::route('dashboard.index') }}"><i class="fa fa-dashboard"></i> {{
            trans('core::core.breadcrumb.home') }}</a></li>
    <li><a href="{{ URL::route('admin.page.page.index') }}">{{ trans('page::posts.posts') }}</a></li>
    <li class="active">{{ trans('page::posts.create post') }}</li>
</ol>
@stop
...
@section('content')
{!! Form::open(['route' => ['admin.post.post.store'], 'method' => 'post']) !!}
        ...
        <div class="nav-tabs-custom">
            ...
            <div class="tab-content">
                ...
                <div class="tab-pane {{ App::getLocale() == $locale ? 'active' : '' }}" id="tab_{{ $i }}">
                    @include('page::admin.posts.partials.create-fields', ['lang' => $locale])
                </div>
                <?php endforeach; ?>
        ...
        <div class="box box-primary">
            <div class="box-body">
                <div class="box-category">
                    {!! Form::label("template", trans('page::posts.form.category_id')) !!}
                    <ul>
                        @foreach ($categories as $category)
                        <li>{!! $category['char'] !!}<input type="checkbox" name="category_id[]" value="{{ $category['id'] }}"> {{ $category['title'] }}</li>
                        @endforeach
                    </ul>
                </div>
                <hr>
        ...
        <div class="box box-primary">
            <div class="box-body">
                <div class='form-group{{ $errors->has("template") ? ' has-error' : '' }}'>
                    ...
                    {!! Form::select("template", $post_templates, old("template", 'post.default'), ['class' =>
