...
class EloquentPageRepository extends EloquentBaseRepository implements PageRepository
{
    public function getListPage()
    {
        ...
        $pages->where('type', 'page');
        return DataTables::of($pages)
            ->addColumn('action', function ($page) {
                return view('page::admin.pages.action', compact('page'));
            })
            ->rawColumns(['action'])
            ->make(true);
    }
    ...
    public function getListPost()
    {
        ...
        return DataTables::of($pages)
            ->addColumn('action', function ($post) {
                return view('page::admin.posts.action', compact('post'));
            })
            ->rawColumns(['action'])
            ->make(true);
    }
    ...
    public function update($model, $data)
    {
        ...
        $category_id = Arr::get($data, 'category_id', []);
        if (count($category_id) > 0) {
            $model->categories()->sync($category_id);
        }
    ...
    public function findPageByCategory($id)
    {
        $posts = Page::whereHas('categories', function ($q) use ($id) {
    ...
    public function getRelatePost($postId, $limit)
    {
        $page = $this->find($postId);
        $categories = $page->categories->pluck('id')->toArray();
        return $this->model->where('type', 'post')->whereHas('categories', function ($q) use ($categories) {
            return $q->whereIn('page__categories.id', $categories);
        })->where('id', '!=', $postId)->limit($limit)->get();
    }

    public function getLatestBlogs($numbers)
    {
        $pages = $this->allWithBuilder();
        return $pages->where('type', 'post')->orderByDesc('created_at')->limit($numbers)->get();
    }
