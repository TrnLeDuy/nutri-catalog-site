    ...
    export default {
        ...
        data() {
            return {
                dialogVisible: false,
                selectedMedia: [],
                selectedOrder: {}, // Add this new property to track order
                eventName: '',
            };
        },
        computed: {
        ...
        mounted() {
            ...
            this.selectedMedia.forEach((media, index) => {
                this.selectedOrder[media.id] = index + 1;
            });
            ...
            this.$events.listen(this.eventName, (mediaData) => {
                if (find(this.selectedMedia, mediaData) === undefined) {
                    ...
                    this.selectedOrder[mediaData.id] = this.selectedMedia.length;
                    this.$emit('multiple-file-selected', { ...mediaData, zone: this.zone, order: this.selectedMedia.length });
                }
            });
        },
        methods: {
            ...
            isMediaSelected(mediaId) {
                return this.selectedMedia.some(media => media.id === mediaId);
            },
            getMediaOrder(mediaId) {
                ...
                axios.get(route('api.media.get-by-zone-and-entity', {
                    ...
                    .then((response) => {
                        ...
                        this.selectedMedia.forEach((file, index) => {
                            this.selectedOrder[file.id] = index + 1;
