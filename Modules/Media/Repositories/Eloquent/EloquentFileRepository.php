...
{
    public function getFileForGird($isWysiwyg, $thumbnails)
    {
        $files = $this->model->select('*');
        $files->where('is_folder', 0);
        return DataTables::of($files)
            ->addColumn('thumbnail', function ($file) {
                return view('media::admin.grid.partials.thumbnail', compact('file'));
            })
            ->addColumn('action', function ($file) use ($isWysiwyg, $thumbnails) {
                return view('media::admin.grid.partials.action', compact('file', 'isWysiwyg', 'thumbnails'));
            })
