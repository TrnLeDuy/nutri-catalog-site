...
class CacheProductDecorator extends BaseCacheDecorator implements ProductRepository
{
    ...
    public function getProductHomepageByCategory($categoryId, $numbers)
    {
        $key = $this->getBaseKey() . "getProductHomepageByCategory" . $categoryId . $numbers;
        return $this->remember(function () use ($categoryId, $numbers) {
            return $this->repository->getProductHomepageByCategory($categoryId, $numbers);
        }, $key);
    }
