...
<div class="form-group">
    ...
    <div class="media">
        ...
        <a class="btn btn-primary btn-browse" onclick="openCustomMediaWindow(event);" <?php echo isset($file) ? 'style="display:none;"' : ''; ?>><i
    ...
    <script>
        if (typeof window.openCustomMediaWindow === 'undefined') {
            ...
            window.openCustomMediaWindow = function(event) {
                const zoneWrapper = $(event.currentTarget).siblings('.jsThumbnailImageWrapper');
                const input = zoneWrapper.find('input');
                window.mediaPanel = $(event.currentTarget).closest('.media');
                window.zoneWrapper = zoneWrapper;
                window.single = true;
                window.old = false;
                window.mediaZone = input.attr('name');
                window.open('{!! route('media.grid.select') !!}', '_blank',
                    'menubar=no,status=no,toolbar=no,scrollbars=yes,height=500,width=1000');
            };
        }
        ...
        $(document).ready(function() {
            ...
            $('.customMedia.jsThumbnailImageWrapper').on('click', '.jsRemoveSimpleLink', function(e) {
                ...
                // Keep the hidden input intact but remove the image and the remove link
                imageValue.closest('figure').find('img').remove();
                imageValue.closest('figure').find('.jsRemoveSimpleLink').remove();

                // Optionally, you can also clear the value of the hidden input to reflect that no image is selected
                imageValue.closest('figure').find('input[type=hidden]').val("");

                // Fade out the figure container after the removal
